...
struct PowerUp
{
    float x, y;
    float size = 0.05f;
    bool active = true; // Whether the power-up is still available
};
vector<PowerUp> healthPowerUps; // Array to store multiple power-ups
...
struct Enemy
{
    float x, y;
    float width = 0.05f, height = 0.1f;
    float lastShootTime = 0.0f; // Track last time this enemy shot a bullet
    int health = 3; // Enemy health, default to 3 (or any value you prefer)
    float moveSpeed = 0.01f; // Speed at which the enemy moves
    float moveRange = 0.3f;  // How far the enemy will move from the starting point
    float startX;            // Starting position of the enemy on the X-axis
    bool movingRight = true; // Direction of movement (right or left)
};
...
struct Coin
{
    float x, y;
    float size = 0.05f;
    bool active = true; // Whether the coin is still available
};
vector<Coin> coins;
...
{
    GameData data = {1, 0};  // Default values: level 1 and high score 0
...
// Append game data to the file for each session
void appendGameData(int currentLevel, int currentScore)
{
    ofstream file("E:\\game_data.txt", ios::app); // Open in append mode
...
bool gunFacingRight = true; // true for right, false for left
// Draw the player
void drawPlayer()
{
    float S = 0.7f; // Scaling factor (0.5 makes the character half its size)
...
void drawHealthBar()
{
    ...
    glColor3f(1.0f, 0.0f, 0.0f); // Background of health bar
    glRectf(x, y, x + barWidth, y + barHeight);

    glColor3f(0.0f, 1.0f, 0.0f); // Health portion
    glRectf(x, y, x + barWidth * (player.health / 5.0f), y + barHeight);
}

// Draw background (simple moving lines for demonstration)
void drawBackground()
{
...
struct LevelBox
{
    float x, y;
    float size = 0.05f;
    bool active = false; // Initially inactive, will be activated when score >= 100
};
LevelBox levelBox = {-0.975f,0.55f}; // Place the level box at a fixed position
...
void drawLevelBox()
{
    if (levelBox.active)
    {
        glColor3f(0.0f, 0.0f, 1.0f); // Blue color for the level box
...
void resetForNextLevel()
{
    ...
    // Set different environment or enemies for the next level
    if (level == 1)
    {
        player.x = -0.9f;
        player.y = -0.875f;
    ...
    // Set the level box to inactive for the new level
...
int getLevelThreshold(int level)
{
    if (level == 1)
    {
        ...
        return 100 ; // Level 1 requires 100 points

    }
    else if (level == 2)
    {
        levelBox.active=true;
        return 200;// Level 2 requires 200 points
    }
    else if (level == 3)
    {
        levelBox.active=true;
        return 300; // Level 3 requires 300 points
    }
    else if (level==4)
    {
        levelBox.active=true;
    ...
    for (size_t i = 0; i < enemyBullets.size(); ++i)
    {
                ...
                enemyBullets[i].y < player.y + player.height)
        {
            ...
            if (player.health <= 0)
            {
                ...
                return; // Exit the checkCollisions() function since the game is over
            }
        }
    }
    if (levelBox.active &&
...
void updatePlayer()
{
    ...
    for (size_t i = 0; i < platforms.size(); ++i)
    {
        ...
        {
            player.y = platforms[i].y;  // Position player on top of the platform
            player.isJumping = false;   // Player is not jumping
            player.yVelocity = 0.0f;    // Stop downward velocity
            onPlatform = true;
            break;
        }
    }

    // If player is not on any platform, apply gravity to make them fall
    if (!onPlatform)
    {
        player.y += player.yVelocity;
        player.yVelocity -= GRAVITY;  // Apply gravity

        // If the player falls below the ground level (assuming ground is at y = -0.8f)
        if (player.y <= GROUND_LEVEL)
        {
            player.y = GROUND_LEVEL;
            player.isJumping = false;
            player.yVelocity = 0.0f;  // Stop falling at ground level
        }

    }

    // Prevent upward movement when not jumping
    if (!player.isJumping && player.y == -0.8f)
    {
        player.yVelocity = 0.0f;  // Ensure no negative velocity when not jumping
    }
}
...
void enemyShoot(Enemy& enemy)
{
    ...
    if (currentTime - enemy.lastShootTime >= 3.0f)   // Check if enough time has passed since last shot
    {
        Bullet enemyBullet;

        // Bullet starts at the center of the enemy
        enemyBullet.x = enemy.x + enemy.width / 2;
        enemyBullet.y = enemy.y + enemy.height / 2;

        // Bullet direction depends on player's position relative to the enemy
...
void updateEnemies()
{
    for (Enemy& enemy : enemies)
    {
        ...
        if (enemy.y - 0.1f <= player.y + player.height && enemy.y + 0.1f >= player.y)
        {
            ...
            {
                enemy.movingRight = false;  // Stop moving horizontally
                enemyShoot(enemy);  // Pass the 'enemy' to the function
            }
        }
        else
        {
...
void Xiaotimer(int)
{
    ...
    for (Enemy& enemy : enemies)
    {
        enemyShoot(enemy);  // Pass the current enemy
    }
